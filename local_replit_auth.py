...
def make_replit_blueprint():
    # For local development, check if REPL_ID is available
    repl_id = os.environ.get('REPL_ID')
    if not repl_id:
        # Create a mock blueprint for local development
        from flask import Blueprint
        replit_bp = Blueprint("replit_auth", __name__)
        
        @replit_bp.route("/login")
        def login():
            # For local development without Replit Auth
            flash('Local development mode: Authentication disabled. Creating demo user.', 'info')
            
            # Create or get demo user
            demo_user = User.query.filter_by(email='demo@local.dev').first()
            if not demo_user:
                demo_user = User()
                demo_user.id = 'demo-user-local'
                demo_user.email = 'demo@local.dev'
                demo_user.first_name = 'Demo'
                demo_user.last_name = 'User'
                demo_user.is_admin = True  # Make demo user admin for testing
...
def require_login(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # Skip token refresh for local development
...
# Create a proxy for replit session, with fallback for local development
def _get_replit_session():
    if hasattr(g, 'flask_dance_replit'):
        return g.flask_dance_replit
    return None

replit = LocalProxy(_get_replit_session)